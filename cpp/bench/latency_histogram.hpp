...
namespace deeplob::bench {

// Bench-only infrastructure -- mirrors the sibling liquibook-x project's own
// bench/latency_histogram.hpp exactly (same reasoning, same interface), not reinvented here.
// Google Benchmark's own model times a whole loop body across many iterations and reports
// mean/median across *repetitions* of that timed region; it doesn't naturally produce a
// percentile distribution of *individual operation* latencies, which is what
// "P50/P99/P99.9" actually means. This records one raw sample per operation and computes
...
// Best-effort CPU pinning for the calling thread. Real on Linux via pthread_setaffinity_np; a
// documented no-op everywhere else -- macOS has no public equivalent thread-affinity API
// (thread_policy_set's affinity tags are a grouping hint, not a guarantee), so this is a
// genuine platform gap, not hidden behind a fake success. Returns whether pinning actually
// took effect, so callers (and BENCHMARKS.md) can report honestly rather than silently
// assuming it worked.
